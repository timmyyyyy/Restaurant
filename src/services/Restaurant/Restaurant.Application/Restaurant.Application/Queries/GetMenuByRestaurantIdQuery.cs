...
public record GetMenuByRestaurantIdQuery(Guid RestaurantId) : IRequest<OperationResult<MenuDto>>;

public class GetMenuByRestaurantIdQueryHandler : IRequestHandler<GetMenuByRestaurantIdQuery, OperationResult<MenuDto>>
{
    ...
    public async Task<OperationResult<MenuDto>> Handle(GetMenuByRestaurantIdQuery request, CancellationToken cancellationToken)
    {
        ...
        if (menuEntity == null)
        {
            return OperationResult<MenuDto>.Failed(
                new NotFoundException($"Menu not found for restaurant {request.RestaurantId}"));
        }
        ...
        var itemsByCategory = menu.Items
            ...
            .Select(y => new MenuCategoryDto
            {
                ...
                Items = [.. y.Select(z => new MenuItemDto
                {
                    ...
                    UnavailabilityReason = z.IsAvailable() ? null : BuildUnavailabilityMessage(z),
                    Availability = [.. z.Availability.Select(a => MapAvailability(a))]
                })]
            }).ToList();

        return OperationResult<MenuDto>.Success(new MenuDto
        {
            Id = menu.Id,
